...
import os.path
import copy
import myDataStructures as myDat
...
def is_palindrome(Stack):
    Stack1=myDat.ArrayStack()
    Stack2compare=myDat.ArrayStack()
    for i in range(len(Stack)):
        Stack2compare.push(Stack.top())
        Stack1.push(Stack.top())
        Stack.pop()
    Stack2=myDat.ArrayStack()
...
def reverseStack(Stack):
    Stack_reverse=myDat.ArrayStack()
    for i in range(len(Stack)):
...
def count_sum(Stack):
    rStackcopy1=myDat.ArrayStack()
    Stackcopy1=myDat.ArrayStack()
