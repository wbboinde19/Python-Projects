...
class Courses():
    ...
    def dropStudent(self,StudentID):
        self.enrollIDList.remove(StudentID)
...
def dropStudents(inputFileName,courseObject):
    inFile=open(inputFileName,"r")
    # if the inputFileName contains IDs to drop and courseObject contains a list of registered students ID
    ID_2drop=inFile.read().split("\n") # Create a list of IDs fromt the file
    cs256CourseObject=Courses("CS",256)
    CourseObject=cs256CourseObject.get_enrollIDList() # Gets the list of all students enrolled in the course
    for a in range(len(ID_2drop)):
        StudentID=ID_2drop[a]
        for i in range(len(CourseObject)):
            student_object=CourseObject[i]
            if StudentID==student_object: # If the two IDs match
                cs256CourseObject.dropStudent(student_ID) # Drop the student in the course
    inFile.close()
    return cs256CourseObject
