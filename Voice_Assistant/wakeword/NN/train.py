...
""" Function: WakeWordData loads and processes wake word data
collate_fn Batches and pads wakeword data""" 
...
def test(test_loader, model, device, epoch):
    ...
    print(pred)
    report = classification_report(labels, preds, zero_division= 1.0)
    print(report)
...
def train(train_loader, model, optimizer, loss_fn, device, epoch):
    ...
    report = classification_report(torch.Tensor(labels).numpy(), torch.Tensor(preds).numpy(), zero_division= 1.0)
    print(report)
    return acc, report
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sample_rate', type=int, default=44100, help='sample_rate for data')
